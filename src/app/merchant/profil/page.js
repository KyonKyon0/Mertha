...
import RoleSwitchLoader from '@/components/ui/RoleSwitchLoader';
...
export default function ProfilPage() {
  ...
  const handleBecomeMerchant = async () => {
    // If the slider is considered ON, we are "sliding off" to return to buyer mode
    // We can assume if they are already in /profil, they might want to go to /merchant if they click it.
    // Let's use the switchingMerchant state for the animation
    ...
    // If DevMode or admin/juri, create dummy merchant immediately
    if (devMode || ['admin', 'super_admin', 'juri'].includes(profile.role)) {
      // Create a dummy merchant
      const newMerchant = {
        user_id: profile.id,
        name: `${profile.name}'s Store`,
        description: "Demo Store generated by dev mode",
        address: "",
        lat: -6.230784,
        lng: 106.818464,
        is_active: true
      ...
      alert("Fitur registrasi Merchant reguler akan segera hadir. Gunakan Developer Mode untuk mencoba sekarang.");
    }
  };
    ...
    <>
      ...
      <main className="flex-1 bg-mertha-bg pb-24 min-h-screen">
        ...
        <section className="bg-white p-6 border-b border-mertha-border mb-3 shadow-sm flex items-center gap-5 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 flex flex-wrap gap-2 items-center">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-mertha-success/10 text-mertha-success rounded-full text-[10px] font-bold uppercase tracking-wider">
        ...
        <section className="bg-white border-y border-mertha-border mb-6 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300 fill-mode-both">
          ...
          {/* Developer Mode */}
          {(['admin', 'super_admin', 'juri'].includes(profile.role)) && (
            <button onClick={() => {
              if (devMode) {
                setDevMode(false);
              ...
              <div className={`w-11 h-6 rounded-full relative cursor-pointer shadow-inner transition-colors ${devMode ? 'bg-amber-500' : 'bg-gray-900'}`}>
          ...
          {(devMode || ['admin', 'super_admin', 'juri'].includes(profile.role) || merchantData) && (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col items-start">
                  <span className="text-sm font-semibold text-mertha-text">
                    Mode Merchant
                  </span>
      ...
      {/* Floating Magic Demos (Only in Dev Mode for Admins & Juri) */}
      {(devMode && ['admin', 'super_admin', 'juri'].includes(profile.role)) && <FloatingTools items={floatingItems} defaultOpen={devOpen} />}
            ...
            <div className="p-6 space-y-5 overflow-y-auto max-h-[60vh]">
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  <label className="block text-xs font-bold text-mertha-subtext uppercase tracking-wider" htmlFor="address">
                    Alamat Rumah
                  </label>
                  <button type="button" onClick={() => { setActiveMapField('address'); setShowMap(true); }} className="text-[10px] font-bold text-mertha-primary flex items-center gap-1 bg-mertha-primary/10 px-2 py-1 rounded-md active:scale-95 transition-transform">
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  <label className="block text-xs font-bold text-mertha-subtext uppercase tracking-wider" htmlFor="office_address">
                    Alamat Kantor
                  </label>
                  <button type="button" onClick={() => { setActiveMapField('office_address'); setShowMap(true); }} className="text-[10px] font-bold text-mertha-primary flex items-center gap-1 bg-mertha-primary/10 px-2 py-1 rounded-md active:scale-95 transition-transform">
                    <MapPin size={12} /> Pilih di Peta
                  </button>
                </div>
                <textarea 
